...
var Scrollbar = (props) => {
  ...
  let thumbRef;
  let trackRef;
  const [thumbSize, setThumbSize] = (0, import_solid_js.createSignal)(50);
  const [thumbPosition, setThumbPosition] = (0, import_solid_js.createSignal)(0);
  const [showScrollbar, setShowScrollbar] = (0, import_solid_js.createSignal)(false);
  const [isDragging, setIsDragging] = (0, import_solid_js.createSignal)(false);
  const [dragOffset, setDragOffset] = (0, import_solid_js.createSignal)(0);
  const [_engineState, _setEngineState] = (0, import_solid_js.createSignal)(null);
  const [_scrollVelocity, _setScrollVelocity] = (0, import_solid_js.createSignal)(0);
