...
export class TauriEngine implements UIEngine {
	...
	async updatePhysicsObject(id: number, x: number, y: number): Promise<void> {
		...
		try {
			await window.__TAURI__!.invoke<void>('update_physics_object', {
				id,
				x,
				y
			})
	...
	async destroyPhysicsObject(id: number): Promise<void> {
		...
		try {
			await window.__TAURI__!.invoke<void>('destroy_physics_object', { id })
	...
	async updateZoomLevel(zoom: number): Promise<void> {
		...
		try {
			await window.__TAURI__!.invoke<void>('update_zoom_level', { zoom })
			this.zoomLevel = zoom
		} catch (error) {
	...
	async screenToWorld(
		...
		try {
			const result = await window.__TAURI__!.invoke<{ x: number; y: number }>(
				'screen_to_world',
				{
					screenX,
					screenY
	...
	async initPhysicsEngine(): Promise<void> {
		...
		try {
			await window.__TAURI__!.invoke<void>('init_physics_engine')
			this.isInitialized = true
		} catch (error) {
