...
export const CodeHighlight: Component<CodeHighlightProps> = (props) => {
  ...
  return (
    <div
      class={props.class}
      style={{
        ...codeHighlightStyles.container(),
        ...props.style,
      }}
    >
      <button
        onClick={copyToClipboard}
        style={codeHighlightStyles.copyButton()}
        onMouseEnter={(e) => {
          e.currentTarget.style.color = "hsl(var(--hover-color))";
        }}
        onMouseLeave={(e) => {
          e.currentTarget.style.color = "hsla(var(--muted-foreground) / 0.8)";
        }}
        title={props.copyButtonTitle ?? "Copy code"}
      >
        {copied()
          ? (props.copiedButtonText ?? "Copied!")
