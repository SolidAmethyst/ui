...
describe('Tabs', () => {
	it('renders with default props', () => {
		render(() => (
			<Tabs preview={<div>Preview Content</div>} code='const x = 1' isDark={() => true} />
		))
	...
	it('shows preview tab by default', () => {
		render(() => (
			<Tabs preview={<div>Preview Content</div>} code='code' isDark={() => true} />
		))
		expect(screen.getByText('Preview Content')).toBeInTheDocument()
	})
	...
	it('handles function isDark prop', () => {
		render(() => (
			<Tabs preview={<div>Preview Content</div>} code='code' isDark={() => false} />
		))
		expect(screen.getByRole('button', { name: 'Preview' })).toBeInTheDocument()
		expect(screen.getByText('Preview Content')).toBeInTheDocument()
	})
