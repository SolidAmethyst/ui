...
describe('Modal', () => {
	it('renders when isOpen is true', async () => {
		render(() => (
			<Modal isOpen={true} onClose={vi.fn()} isDark={false}>
	...
	it('does not render content when isOpen is false', () => {
		render(() => (
			<Modal isOpen={false} onClose={vi.fn()} isDark={false}>
				<div>Modal Content</div>
			</Modal>
		))
	...
	it('calls onClose when backdrop is clicked', async () => {
		const onClose = vi.fn()
		render(() => (
			<Modal isOpen={true} onClose={onClose} isDark={false}>
				<div>Content</div>
			</Modal>
		))
		...
		await waitFor(
			() => {
				...
				const backdrop = Array.from(backdrops).find(
					el => el.getAttribute('style')?.includes('background: rgba(0, 0, 0, 0.5)')
				)
	...
	it('applies custom z-index', async () => {
		render(() => (
			<Modal isOpen={true} onClose={vi.fn()} zIndex={15000} isDark={false}>
				<div>Content</div>
			</Modal>
		))
	...
	it('does not trap focus when trapFocus is false', async () => {
		render(() => (
			<Modal isOpen={true} onClose={vi.fn()} trapFocus={false} isDark={false}>
				<div>
