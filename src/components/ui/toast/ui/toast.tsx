...
export const Toast: Component<ToastProps> = (props) => {
  ...
  return (
    <div
      ...
      style={toastStyles.toast(props.toast.variant, isVisible())}
    >
      <span
        class="material-symbols-rounded"
        style={toastStyles.icon(props.toast.variant)}
      >
        {getIcon()}
      </span>
      <div style={toastStyles.content()}>
        <Show when={props.toast.title}>
          <div style={toastStyles.title()}>{props.toast.title}</div>
        </Show>
        <Show when={props.toast.description}>
          <div style={toastStyles.description()}>{props.toast.description}</div>
        </Show>
      </div>
      <button
        type="button"
