...
export const Timeline: Component<TimelineProps> = (props) => {
  ...
  return (
    <div
      ...
      style={{
        ...timelineStyles.container(orientation()),
        ...props.style,
      }}
    >
      <For each={props.events}>
        {(event, index) => {
          ...
          return (
            <div style={timelineStyles.event(orientation(), showLine())}>
              <Show when={showLine()}>
                <div style={timelineStyles.line(orientation(), isLast())} />
              </Show>
              <div
              ...
              <div style={timelineStyles.content(orientation())}>
                <Show when={event.title}>
                  <h3 style={timelineStyles.title(event.disabled ?? false)}>
                    {event.title}
                  </h3>
                </Show>
                <Show when={event.description}>
                  <div
                    style={timelineStyles.description(event.disabled ?? false)}
                  >
                    {event.description}
                  </div>
                </Show>
                <Show when={showDate() && event.date}>
                  <div style={timelineStyles.date(event.disabled ?? false)}>
                    {event.date}
                  </div>
                </Show>
