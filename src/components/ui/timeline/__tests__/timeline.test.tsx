...
describe('Timeline', () => {
	...
	it('renders timeline with events', () => {
		render(() => <Timeline events={mockEvents} isDark={false} />)
	...
	it('renders event dates when showDate is true', () => {
		render(() => <Timeline events={mockEvents} showDate={true} isDark={false} />)
		expect(screen.getByText('2024-01-01')).toBeInTheDocument()
		expect(screen.getByText('2024-01-02')).toBeInTheDocument()
	})

	it('hides event dates when showDate is false', () => {
		render(() => <Timeline events={mockEvents} showDate={false} isDark={false} />)
	...
	it('renders with horizontal orientation', () => {
		render(() => (
			<Timeline events={mockEvents} orientation='horizontal' isDark={false} />
		))
		const container = screen.getByText('Event 1').closest('.timeline')
	...
	it('renders custom icon when provided', () => {
		...
		render(() => <Timeline events={eventsWithIcon} isDark={false} />)
	...
	it('renders default icon based on variant', () => {
		...
		render(() => <Timeline events={eventsWithVariant} isDark={false} />)
		expect(screen.getByText('check_circle')).toBeInTheDocument()
	})

	it('applies custom class name', () => {
		render(() => (
			<Timeline events={mockEvents} class='custom-timeline' isDark={false} />
		))
		const container = screen.getByText('Event 1').closest('.timeline')
	...
	it('renders custom content when provided', () => {
		...
		render(() => <Timeline events={eventsWithContent} isDark={false} />)
