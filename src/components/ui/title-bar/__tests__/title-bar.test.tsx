...
describe("TitleBar", () => {
  ...
  it("renders burger menu button when onBurgerClick is provided", () => {
    ...
    const burgerButton = screen.getByTitle("Menu");
  ...
  it("shows light mode icon when isDark is false", () => {
    render(() => <TitleBar title="Test" onThemeToggle={() => {}} isDark={false} />);
    const themeButton = screen.getByTitle("Dark mode");
    expect(themeButton).toBeInTheDocument();
  });

  it("shows dark mode icon when isDark is true", () => {
    render(() => <TitleBar title="Test" onThemeToggle={() => {}} isDark={true} />);
