...
describe("Select", () => {
  ...
  it("calls onChange when option is selected", () => {
    ...
    render(() => (
      <Select options={options} value="option1" onChange={onChange} />
    ));
    const trigger = screen.getByText("Option 1").closest("button");
    fireEvent.click(trigger!);
  ...
  it("renders with description", () => {
    ...
    render(() => (
      <Select options={optionsWithDesc} value="opt1" onChange={() => {}} />
    ));
    const trigger = screen.getByText("Option 1").closest("button");
    fireEvent.click(trigger!);
  ...
  it("handles disabled option", () => {
    ...
    const onChange = vi.fn();
    render(() => (
      <Select options={optionsWithDisabled} value="opt1" onChange={onChange} />
    ));
    const trigger = screen.getByText("Option 1").closest("button");
    fireEvent.click(trigger!);
