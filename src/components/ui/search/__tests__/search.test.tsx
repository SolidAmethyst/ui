...
describe('Search', () => {
	...
	it('renders with custom placeholder', () => {
		render(() => <Search placeholder="Search users..." isDark={false} />)
		expect(screen.getByPlaceholderText('Search users...')).toBeInTheDocument()
	})

	it('renders with initial value', () => {
		render(() => <Search value="test query" isDark={false} />)
	...
	it('hides search icon when showIcon is false', () => {
		render(() => <Search showIcon={false} isDark={false} />)
		expect(screen.queryByText('search')).not.toBeInTheDocument()
	})

	it('shows clear button when value is present', () => {
		render(() => <Search value="test" isDark={false} />)
	...
	it('hides clear button when value is empty', () => {
		render(() => <Search value="" isDark={false} />)
		expect(screen.queryByLabelText('Clear search')).not.toBeInTheDocument()
	})

	it('hides clear button when showClear is false', () => {
		render(() => <Search value="test" showClear={false} isDark={false} />)
		expect(screen.queryByLabelText('Clear search')).not.toBeInTheDocument()
	})

	it('calls onInput immediately on input change', () => {
		const onInput = vi.fn()
		render(() => <Search onInput={onInput} isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
	...
	it('calls onSearch after debounce delay', async () => {
		...
		render(() => <Search onSearch={onSearch} debounceMs={300} isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
	...
	it('uses default debounce delay of 300ms', async () => {
		const onSearch = vi.fn()
		render(() => <Search onSearch={onSearch} isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
	...
	it('clears value when clear button is clicked', async () => {
		const onSearch = vi.fn()
		render(() => <Search value="test" onSearch={onSearch} isDark={false} />)
		const clearButton = screen.getByLabelText('Clear search')
	...
	it('renders disabled state', () => {
		render(() => <Search disabled isDark={false} />)
	...
	it('does not call callbacks when disabled', () => {
		...
		const onSearch = vi.fn()
		render(() => (
			<Search disabled onInput={onInput} onSearch={onSearch} isDark={false} />
		))
		const input = screen.getByPlaceholderText('Search...')
	...
	it('applies custom class name', () => {
		render(() => <Search class="custom-search" isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
	...
	it('applies custom inline styles', () => {
		render(() => <Search style={{ width: '500px' }} isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
		const container = input.closest('div')?.parentElement
	...
	it('supports name attribute', () => {
		render(() => <Search name="search-query" isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
		expect(input).toHaveAttribute('name', 'search-query')
	})

	it('supports id attribute', () => {
		render(() => <Search id="search-input" isDark={false} />)
		const input = screen.getByPlaceholderText('Search...')
		expect(input).toHaveAttribute('id', 'search-input')
	})

	it('handles boolean isDark prop', () => {
		render(() => <Search isDark={true} />)
		const input = screen.getByPlaceholderText('Search...')
		expect(input).toBeInTheDocument()
	})
	...
	it('syncs with external value prop', () => {
		render(() => <Search value="initial" isDark={false} />)
