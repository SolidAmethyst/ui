...
export const Scrollbar = (props: ScrollbarProps) => {
	...
	// Engine state for physics-based scrolling (unused, reserved for future use)
	// const [engineState, setEngineState] = createSignal<unknown>(null);
	...
	const updateScrollbar = () => {
		...
		// Simple logic WITHOUT ScrollContext
	...
	const handleThumbMouseDown = async (e: MouseEvent) => {
		...
		// Precise logic: offset from mouse to THUMB CENTER
		const trackRect = trackRef.getBoundingClientRect()
		// Account for zoom - divide by zoom level
		...
		// Offset from mouse to THUMB CENTER (like cube logic with -25)
	...
	const handleMouseMove = async (e: MouseEvent) => {
		...
		// Precise logic: position = mouse - offset
		const trackRect = trackRef.getBoundingClientRect()
		// Account for zoom - divide by zoom level
		const zoomLevel = parseFloat(
			getComputedStyle(document.documentElement).getPropertyValue(
		...
		// THUMB CENTER position = mouse position - offset (like cube logic)
