...
export const FileManager = (props: FileManagerProps) => {
	...
	const isDark = () => local.isDark ?? true
	...
	const [mainCheckboxIndeterminate, setMainCheckboxIndeterminate] = createSignal(false)
	...
	createEffect(() => {
		...
		const selectedCount = allFiles.filter(item => selected.includes(item.path))
			.length
							...
							{drive => (
								<button
									...
									onMouseLeave={e => {
										...
										e.currentTarget.style.background = normalStyles.background as string
						...
						{(part, index) => (
							<>
								<button
									...
									onMouseEnter={e => {
										const hoverStyles = fileManagerStyles.breadcrumbButtonHover()
										e.currentTarget.style.background = hoverStyles.background as string
										e.currentTarget.style.color = hoverStyles.color as string
									}}
									onMouseLeave={e => {
					...
					<div style={fileManagerStyles.searchControls()}>
						<Show when={searchQuery().length > 0}>
							<button
								...
								onMouseEnter={e => {
									const hoverStyles = fileManagerStyles.searchClearButtonHover()
									e.currentTarget.style.background = hoverStyles.background as string
								}}
								onMouseLeave={e => {
									...
									{(item, index) => {
										...
										return (
											<tr
												...
												onMouseEnter={e => {
													const hoverStyles = fileManagerStyles.trHover(selected())
													e.currentTarget.style.background = hoverStyles.background as string
												}}
												onMouseLeave={e => {
												...
												onClick={e => {
													if (isFile) {
														...
														handleFileClick(item, fileIndex, e, filteredItems())
													}
												}}
											...
											>
												...
												<td style={fileManagerStyles.td()}>
													...
													>
														<svg
															style={fileManagerStyles.fileIcon(
																item.isFolder
															)}
															fill={item.isFolder ? 'currentColor' : 'currentColor'}
															viewBox={item.isFolder ? '0 0 20 20' : '0 0 20 20'}
														>
															{item.isFolder ? (
