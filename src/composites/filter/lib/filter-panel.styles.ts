...
export const filterPanelStyles = {
	...
	checkboxListContainer: (isDark?: boolean): JSX.CSSProperties => ({
		background: isDark
			? 'hsl(240 15% 10%)'
			: 'hsl(var(--secondary))',
		border: isDark
			? '1px solid hsl(240 14% 18%)' // Slightly lighter than --muted for better visibility
	...
	selectAllContainer: (isDark?: boolean): JSX.CSSProperties => ({
	...
	checkboxItemHover: (isDark?: boolean): JSX.CSSProperties => ({
		'background-color': isDark
			? 'hsla(var(--primary-hover) / 0.18)'
			: 'hsla(var(--primary-hover) / 0.1)',
		color: 'hsl(var(--foreground))'
	...
	sizeInput: (isDark?: boolean): JSX.CSSProperties => ({
		width: '100%',
		'box-sizing': 'border-box',
		background: isDark
			? 'hsl(240 15% 10%)' // Explicit dark theme value
			: 'hsl(var(--secondary))',
		border: isDark
			? '1px solid rgba(255, 255, 255, 0.1)'
			: '1px solid hsl(var(--border))',
		color: isDark ? '#f6f6f6' : 'hsl(var(--foreground))'
	}),
