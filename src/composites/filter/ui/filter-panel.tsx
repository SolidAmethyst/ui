...
import type { FileTypeFilter, FilterPanelProps } from '../model/filter-panel.types'
...
export const FilterPanel: Component<FilterPanelProps> = props => {
	...
	const [showHidden, setShowHidden] = createSignal(props.showHiddenFiles ?? false)
		...
		>
			<div style={filterPanelStyles.container()}>
				...
				<div style={filterPanelStyles.header()}>
					<Typography variant='h6' isDark={isDark()} style={filterPanelStyles.title()}>
						{title()}
					</Typography>
					<button
				...
				<div style={filterPanelStyles.content()}>
					...
					<Show when={fileTypes().length > 0}>
						<div style={filterPanelStyles.section()}>
							<Typography variant='small' isDark={isDark()} style={filterPanelStyles.sectionTitle()}>
								File Types
							</Typography>
							<div style={filterPanelStyles.checkboxListContainer(isDark())}>
								<div
									style={filterPanelStyles.selectAllContainer(isDark())}
								>
									<Checkbox
									...
									<For each={fileTypes()}>
										{fileType => {
											...
											const baseItemStyle = () => {
												...
												if (isSelected()) {
													const selected = filterPanelStyles.checkboxItemSelected()
												...
												<div
													...
													onMouseEnter={e => {
														const hoverStyles = isSelected()
															...
															: filterPanelStyles.checkboxItemHover(isDark())
														Object.keys(hoverStyles).forEach(key => {
															e.currentTarget.style[key as any] = hoverStyles[key as keyof typeof hoverStyles] as string
														})
													}}
													onMouseLeave={e => {
														const baseStyles = baseItemStyle()
														Object.keys(baseStyles).forEach(key => {
															e.currentTarget.style[key as any] = baseStyles[key as keyof typeof baseStyles] as string
														})
													}}
												>
													<Checkbox
						...
						<div style={filterPanelStyles.sizeInputs()}>
							<div style={filterPanelStyles.sizeInputGroup()}>
								...
								<NumberInput
									...
									min={0}
									themeAware={false}
									style={filterPanelStyles.sizeInput(isDark())}
								/>
							</div>
							<div style={filterPanelStyles.sizeInputGroup()}>
